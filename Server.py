...
def postData():
    ...
    class userTable(userdb.Model):
        if userName is not '':
            ...
            picuri= userdb.Column(userdb.String)
            minipicuri= userdb.Column(userdb.String)
            extend_existing = True
        else:
    ...
    cPicUri = request.form['picuri']
    cMiniPicUri = request.form['minipicuri']
    cBillName = request.form['billname']
    try:
        userData = userTable(name=cName, money=cMoney, describe=cDescribe, picuri=cPicUri, date=cDate, minipicuri=cMiniPicUri,
                             billname=cBillName)
...
def getBillBean():
    ...
    for i in range(billBeanList.__len__()):
        ...
        bean['picuri'] = billBeanList.__getitem__(i).picuri
        bean['minipicuri'] = billBeanList.__getitem__(i).minipicuri
...
if __name__ == '__main__':
    # manager.run()
    app.run(host='120.25.232.120', port=8070,threaded=True)
